...
All of this started when I filed my first patent. This blog has nothing to _do_ with said patent, beyond the end result ultimately being a part of a version 4 prototype, but this fun thing wouldn't exist without the patent existing first. 
...
So first of all yes this isn't a roomba, but that's what this little fellow _wants_ to be. I picked this imposter up at a yard sale for $20. I had no clue Hoover made robot vacuums and had a hope that Hoover wouldn't put too much work making custom components to build one. The more off the shelf components built in the easier it'd be for me to repurpose. I didn't have time to research exactly what it was before buying it, and it turned out to be a mid range model that had a surprising number of features and not great product reviews. 

After snatching the little thing up it sat around for a few months while I decided exactly what to do with it. Eventually it would be the new prototype for a method of mapping rooms (soon to be open sourced and blogged about), but I wanted to kick the tires on this little guy first. So before going through the effort of taking the existing prototype apart, designing new structures for components to sit on and integrating it into this vacuum I thought it'd be fun to get it working with an old Playstation controller so it could drive it around, vacuum some stuff up and try to improve its (at the time) fairly sad performance. 
...
For a bit I did go down the road of trying to read the chips memory and possibly reprogram it with my own code (which would be _very_ nice since the board has connections to all of the components) but ultimately decided not too because:    
